...
De esta manera, la estimación puede expresarse como: $$\hat{y}_t = \frac{1}{k}\sum_{i=1}^{k} y_i$$
donde:
...
La estimación del valor faltante está dada por: $$\hat{y}_t = \frac{\sum_{i=1}^{k} w_i y_i}{\sum_{i=1}^{k} w_i}$$ donde:
...
La estimación por media móvil simple es un caso particular, en el cual: $w_i = \frac{1}{k}, \quad \forall i$. Por lo tanto, la media móvil lineal ponderada constituye una generalización de la media móvil simple.
...
La expresión del valor imputado resulta igual a la imputación usando el método media móvil lineal ponderada. Sin embargo, en esta técnica, los pesos decrecen exponencialmente con la distancia temporal. Una forma habitual de representarlos es: $$w_i = \alpha (1-\alpha)^{i-1},  \quad 0 < \alpha < 1 $$ donde $\alpha$ es el parámetro de suavizamiento, encargado de controlar la rapidez con la que disminuyen los pesos. Valores elevados de $\alpha$ otorgan mayor importancia a las observaciones más recientes, mientras que valores pequeños producen un suavizado más paulatino. 
...
Un modelo autorregresivo de orden $p$, denotado como $AR(p)$, supone que cada observación depende linealmente de las p observaciones inmediatamente anteriores. De esta manera, la dinámica de la serie queda determinada por su propia historia reciente. Formalmente, un proceso AR(p) puede expresarse como: $$Y_t=c+{\phi}_1Y_{t-1}+{\phi}_2Y_{t-2}+\dots+{\phi}_pY_{t-p}+a_t $$ donde:
...
Un proceso de media móvil de orden $q$, denotado como $MA(q)$, supone que el valor observado en cada instante temporal depende de los errores ocurridos en los $q$ períodos anteriores. La ecuación resulta: $$Y_t=\mu+\theta_1a_{t-1}+\theta_2a_{t-2}+\dots+\theta_qa_{t-q}$$ donde:
...
En un modelo $ARMA(p,q)$, el valor actual de la serie temporal se expresa como una combinación lineal de las $p$ observaciones previas y de los $q$ errores aleatorios pasados. Formalmente, se expresa como: $$Y_t=c+\sum_{i=1}^p\phi_iY_{t-i}+\sum_{j=1}^q\theta_ja_{t-j}$$ cuyos componentes ya fueron definidos anteriormente.
