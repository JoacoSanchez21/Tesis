...
### Evaluación del desempeño de los métodos de imputación
...
### Evaluación del desempeño del pronóstico posterior
...
Con el objetivo de evaluar el impacto de distintos métodos de imputación sobre el desempeño de modelos de pronóstico en series temporales univariadas, se plantea un procedimiento experimental basado en la introducción controlada de datos faltantes en series originalmente completas. Esta estrategia permite conocer en todo momento los valores reales de las observaciones eliminadas y, en consecuencia, evaluar la calidad de las imputaciones realizadas y su efecto sobre la precisión de los pronósticos.

Se selecciona una serie temporal univariada completa, es decir, sin valores faltantes, y sobre esta se realiza un análisis exploratorio inicial con el fin de identificar sus principales características, tales como la presencia de tendencia, estacionalidad, posibles valores atípicos y estructura de autocorrelación. Asimismo, cada serie se divide en un conjunto de entrenamiento y un conjunto de prueba. El conjunto de entrenamiento se utiliza para realizar la imputación de los datos faltantes y ajustar los modelos de pronóstico, mientras que el conjunto de prueba permanece sin modificaciones y se emplea exclusivamente para evaluar la capacidad predictiva de los modelos.

A partir de las series de entrenamiento se generan artificialmente distintos escenarios de datos faltantes. Para ello se consideran diferentes porcentajes de observaciones ausentes y distintos mecanismos de generación de datos faltantes, reproduciendo situaciones que pueden presentarse en aplicaciones reales.

Una vez generados los escenarios de datos faltantes, se aplican los diferentes métodos de imputación considerados para reconstruir las observaciones ausentes. Con el propósito de reducir el efecto del azar asociado a la ubicación de los datos faltantes, cada combinación de serie, mecanismo y porcentaje de faltantes se replica 100 veces utilizando distintas configuraciones de eliminación aleatoria. De esta manera, los resultados obtenidos corresponden al promedio de un elevado número de iteraciones, proporcionando estimaciones más estables del desempeño de cada método.

La calidad de las imputaciones se evalúa comparando los valores imputados con las observaciones originales eliminadas artificialmente. Para cuantificar la precisión de cada método se utilizan las métricas RMSE, MAE y MAPE, anteriormente definidas.

Luego, utilizando las series imputadas, se ajustan los modelos de pronóstico considerados. Los pronósticos se generan para horizontes de h=1, 3, 6 y 12 períodos hacia adelante. Finalmente, el desempeño predictivo de cada modelo se evalúa comparando los valores pronosticados con las observaciones reales del conjunto de prueba mediante las mismas métricas de error utilizadas para evaluar la imputación. Esto permite analizar no solo la capacidad de cada método para reconstruir los valores faltantes, sino también el impacto que dichas imputaciones tienen sobre la precisión de los pronósticos posteriores.

El procedimiento completo puede resumirse en las siguientes etapas:

1 - Selección de la serie temporal. Se seleccionan series temporales univariadas completas, que servirán como referencia para la generación de los escenarios experimentales.

2 - Análisis exploratorio de la serie. Se estudian las principales características de cada serie, incluyendo tendencia, estacionalidad, autocorrelación y posibles valores atípicos. Asimismo, se define la partición entre conjunto de entrenamiento y conjunto de prueba.

3 - Generación de datos faltantes. Se introducen valores faltantes de forma artificial sobre el conjunto de entrenamiento, considerando distintos porcentajes y mecanismos de generación de faltantes.

4 - Aplicación de los métodos de imputación. Se imputan las observaciones faltantes mediante las técnicas consideradas en el estudio.

5 - Evaluación de la imputación. Se comparan los valores imputados con los valores originales mediante las métricas RMSE, MAE y MAPE.
Ajuste de los modelos de pronóstico. Se ajustan los modelos ARIMA/SARIMA y ETS utilizando las series imputadas, mientras que Prophet se ajusta directamente sobre la serie con datos faltantes.

6 - Generación de pronósticos. Se obtienen predicciones para horizontes de h=1, 3, 6 y 12.

7 - Evaluación del pronóstico. Se comparan los pronósticos con las observaciones del conjunto de prueba utilizando las métricas RMSE, MAE y MAPE.

8 - Comparación de resultados. Se analizan conjuntamente los errores de imputación y de pronóstico para determinar el efecto que tienen los distintos métodos de imputación sobre el desempeño predictivo de cada modelo.


## Caso de aplicación: Consumo de electricidad {.unnumbered}
...
En 2020 se observa un evento atípico: un quiebre o shock estructural asociado a la pandemia de COVID-19. Por otro lado, en marzo de 2023 se observa un pico de demanda asociado a condiciones climáticas extremas (ola de calor).
...
```{r tabla5, results='asis'}
tabla <- readRDS("Resultados/resultados_MCAR_25_ARIMA_h3.rds")
...
tabla %>%
  kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "Errores de imputación (Escenario 25\\% MCAR)",
    col.names = c("Método", "RMSE", "MAE", "MAPE")
  ) %>%
  kable_styling(
    full_width = FALSE,
...
```{r tabla6, results='asis'}
h3  <- readRDS("Resultados/resultados_MCAR_25_ARIMA_h3.rds")
h6  <- readRDS("Resultados/resultados_MCAR_25_ARIMA_h6.rds")
h12 <- readRDS("Resultados/resultados_MCAR_25_ARIMA_h12.rds")
...
tabla %>%
  kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "25\\% MCAR, pronosticando con ARIMA",
    col.names = c(
      "Método",
      "RMSE", "MAE", "MAPE",
      "RMSE", "MAE", "MAPE",
...
```{r tabla7, results='asis'}
h3  <- readRDS("Resultados/resultados_MCAR_25_ETS_h3.rds")
h6  <- readRDS("Resultados/resultados_MCAR_25_ETS_h6.rds")
h12 <- readRDS("Resultados/resultados_MCAR_25_ETS_h12.rds")
...
tabla %>%
  kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "25\\% MCAR, pronosticando con ETS",
    col.names = c(
      "Método",
      "RMSE", "MAE", "MAPE",
      "RMSE", "MAE", "MAPE",
...
```{r tabla8, results='asis'}
h3  <- readRDS("Resultados/resultados_MCAR_25_PROPHET_h3.rds")
h6  <- readRDS("Resultados/resultados_MCAR_25_PROPHET_h6.rds")
h12 <- readRDS("Resultados/resultados_MCAR_25_PROPHET_h12.rds")
...
tabla %>%
  ...
  kable(
    ...
    align = "lccccccccc",
    booktabs = TRUE,
    caption = "25\\% MCAR, pronosticando con Prophet",
    col.names = c(
      "Modelo",
      "RMSE", "MAE", "MAPE",
      "RMSE", "MAE", "MAPE",
...
```{r tabla9, results='asis'}
tabla <- readRDS("Resultados/resultados_MCAR_40_ARIMA_h3.rds")
...
tabla %>%
  kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "Errores de imputación (Escenario 40\\% MCAR)",
    col.names = c("Método", "RMSE", "MAE", "MAPE")
  ) %>%
  kable_styling(
    full_width = FALSE,
...
```{r tabla10, results='asis'}
h3  <- readRDS("Resultados/resultados_MCAR_40_ARIMA_h3.rds")
h6  <- readRDS("Resultados/resultados_MCAR_40_ARIMA_h6.rds")
h12 <- readRDS("Resultados/resultados_MCAR_40_ARIMA_h12.rds")
...
tabla %>%
  kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "40\\% MCAR, pronosticando con ARIMA",
    col.names = c(
      "Método",
      "RMSE", "MAE", "MAPE",
      "RMSE", "MAE", "MAPE",
...
```{r tabla11, results='asis'}
h3  <- readRDS("Resultados/resultados_MCAR_40_ETS_h3.rds")
h6  <- readRDS("Resultados/resultados_MCAR_40_ETS_h6.rds")
h12 <- readRDS("Resultados/resultados_MCAR_40_ETS_h12.rds")
...
tabla %>%
  kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "40\\% MCAR, pronosticando con ETS",
    col.names = c(
      "Método",
      "RMSE", "MAE", "MAPE",
      "RMSE", "MAE", "MAPE",
...
```{r tabla12, results='asis'}
h3  <- readRDS("Resultados/resultados_MCAR_40_PROPHET_h3.rds")
h6  <- readRDS("Resultados/resultados_MCAR_40_PROPHET_h6.rds")
h12 <- readRDS("Resultados/resultados_MCAR_40_PROPHET_h12.rds")
...
tabla %>%
  ...
  kable(
    ...
    align = "lccccccccc",
    booktabs = TRUE,
    caption = "40\\% MCAR, pronosticando con Prophet",
    col.names = c(
      "Modelo",
      "RMSE", "MAE", "MAPE",
      "RMSE", "MAE", "MAPE",
